    ...
    <div className="flex flex-col min-h-screen bg-white">
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-start justify-center h-full max-w-7xl mx-auto px-6 md:px-12 text-white">
              <div className="overflow-hidden">
                <h1 className={`text-6xl md:text-8xl font-black mb-6 uppercase tracking-tighter ${i === carouselIndex ? 'animate-slideUp' : ''}`}>
                  {slide.title.split(' ').map((word, idx) => (
                    <span key={idx} className={idx === 1 ? "text-primary italic" : ""}>{word} </span>
                  ))}
                </h1>
              </div>
              <p className={`text-xl md:text-2xl mb-10 max-w-xl font-medium opacity-90 leading-relaxed ${i === carouselIndex ? 'animate-fadeIn delay-500' : ''}`}>
                {slide.subtitle}
              </p>
              <div className="flex gap-6">
                <Link href="/properties" className="bg-primary hover:bg-white hover:text-primary text-white px-10 py-4 rounded-full font-black text-lg transition-all shadow-2xl hover:shadow-primary/40 transform hover:-translate-y-1">
                  START EXPLORING
                </Link>
                <Link href="/register" className="glass-morphism text-white border-2 border-white/30 hover:bg-white/10 px-10 py-4 rounded-full font-black text-lg transition-all backdrop-blur-md">
                  JOIN COMMUNITY
                </Link>
              </div>
            </div>
      ...
      <section className="py-12 bg-black text-white">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-8 text-center">
          <div><p className="text-4xl font-black text-primary">500+</p><p className="text-xs uppercase tracking-widest text-gray-400 mt-2">Smart Units</p></div>
          <div><p className="text-4xl font-black text-primary">12k+</p><p className="text-xs uppercase tracking-widest text-gray-400 mt-2">Happy Tenants</p></div>
          <div><p className="text-4xl font-black text-primary">99%</p><p className="text-xs uppercase tracking-widest text-gray-400 mt-2">Security Rate</p></div>
          <div><p className="text-4xl font-black text-primary">24h</p><p className="text-xs uppercase tracking-widest text-gray-400 mt-2">Smart Support</p></div>
        </div>
      </section>
        ...
        <section className="py-24 bg-white">
          <div className="max-w-7xl mx-auto px-6">
            <div className="flex flex-col lg:flex-row items-center gap-16">
              <div className="lg:w-3/5 group relative overflow-hidden rounded-[2.5rem] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)]">
                <img
                ...
                <div className="absolute top-8 left-8 bg-white/90 backdrop-blur-md px-6 py-2 rounded-full shadow-xl">
                  <span className="text-sm font-black text-primary uppercase tracking-widest">Featured Listing</span>
                </div>
              </div>
              <div className="lg:w-2/5 flex flex-col items-start">
                <h2 className="text-5xl font-black text-gray-900 mb-6 leading-tight uppercase tracking-tighter">
                  Experience The<br /><span className="text-primary italic">Elite Standard</span>
                </h2>
                <h3 className="text-2xl font-bold text-gray-800 mb-4">{spotlightProperty.title}</h3>
                ...
                <div className="flex flex-col gap-6 w-full">
                  <div className="flex gap-4 border-b border-gray-100 pb-6">
                    <div className="bg-gray-50 p-4 rounded-2xl flex-1 text-center">
                      ...
                      <p className="text-xl font-black text-gray-900">{spotlightProperty.pricing.currency === 'NGN' ? '₦' : '$'}{spotlightProperty.pricing.rentPrice?.toLocaleString() || spotlightProperty.pricing.salePrice?.toLocaleString()}</p>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-2xl flex-1 text-center">
                      <p className="text-xs font-bold text-gray-400 uppercase mb-1">Type</p>
                      <p className="text-xl font-black text-gray-900 uppercase tracking-tighter">{spotlightProperty.listingType?.replace('_', ' ')}</p>
                    </div>
                  </div>
      ...
      <section className="py-24 bg-gray-50 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
            <div>
              <span className="text-primary font-black uppercase tracking-[0.3em] text-sm">Our Portfolio</span>
                ...
                {[
                  ...
                  <div key={i} className="bg-white/10 backdrop-blur-lg p-10 rounded-[2rem] border border-white/10 flex flex-col justify-between hover:bg-white/15 transition-all">
                    ...
                    <div className="flex items-center gap-4 border-t border-white/10 pt-6">
                      <div className="w-12 h-12 bg-white text-primary rounded-2xl flex items-center justify-center font-bold shadow-lg">
                        {t.author.charAt(0)}
                      </div>
                      <div>
